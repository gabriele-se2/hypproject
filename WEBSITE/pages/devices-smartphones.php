...
function get_json()
{
    ...
    if (!count($queries)) {
        $where_clause = "";
        $result = $db->query("SELECT * FROM smartphones " . $join . " ORDER BY manufacturer, name " . $limit);
        ...
        $final_query = "SELECT * FROM smartphones " . $join . " " . $where_clause .
...
{
    $db = Database::getInstance();
    $query = "SELECT DISTINCT manufacturer FROM smartphones JOIN devices USING(id)";
...
{
    $db = Database::getInstance();
    $query = "SELECT DISTINCT os FROM smartphones";
    $result = $db->query($query);
    while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
        $data[] = $row["os"];
    }
...
function get_page_count($where_clause = "", $types = null, $values = null)
{
    ...
    if ($types == null || $values == null) {
        $query = "SELECT COUNT(*) FROM smartphones " . $where_clause;
        $result = $db->query($query);
    } else {
        $query = "SELECT COUNT(*) FROM smartphones JOIN devices USING(id) " . $where_clause;
